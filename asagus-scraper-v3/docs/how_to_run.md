...
- Extraction cascade
- Enrichment
- Local file-backed records when real fetch stores useful leads
- Hybrid search
- LLM provider setup
...
Keep these `false` for first run. The app will use offline preview data so you can test the pipeline safely; preview pages are skipped and are not stored as business leads.
...
8. In preview mode, expect zero stored leads; enable real fetch/discovery for real records
9. Try `Search`
...
When `Respect robots` is checked for a real-fetch job, the backend fetches and caches each domain's robots.txt before crawling. `Archive HTML` saves raw pages under `data/raw_html/<job-id>/`.

Optional bring-your-own proxy URLs can be set in `.env`:
