...
class LLMSettings(BaseModel):
    ...
    @classmethod
    def handle_unknown_provider(cls, data: dict[str, Any]) -> dict[str, Any]:
        if isinstance(data, dict) and "provider" in data:
            provider_val = data["provider"]
            # If provider is not a valid LLMProvider enum value, default to disabled
            if isinstance(provider_val, str):
                try:
                    LLMProvider(provider_val)
                except ValueError:
                    data["provider"] = LLMProvider.disabled.value
        return data
...
class ScrapeStartRequest(BaseModel):
    ...
    mode: Literal["balanced", "fast", "deep", "deep_agent", "parallel", "research", "focused", "comprehensive", "adaptive", "max"] = "balanced"
    # Anti-bot research preset for selecting the browser/client strategy used by
    # the active fetch path. It does not bypass challenges; challenge pages are
    # still routed to manual review when configured.
    antibot_preset: Literal["high-stealth", "balanced", "high-speed"] = Field(
        default="balanced",
        description="Educational/research anti-bot preset: 'high-stealth' prefers Camoufox if installed, "
                    "'balanced' prefers Patchright if installed, and 'high-speed' uses the fastest local path. "
                    "Optional adapters fall back to Playwright and no CAPTCHA/challenge bypass is attempted."
    )
    discovery_mode: Literal["website_first", "social_first", "social_only"] = "website_first"
    lead_target: Literal["businesses", "decision_makers"] = "businesses"
    website_filter: Literal["any", "has_website", "no_website"] = "any"
    decision_maker_titles: str = Field(default="owner, founder, CEO", max_length=160)
    allowed_domains: list[str] = Field(default_factory=list)
    blocked_domains: list[str] = Field(default_factory=list)
    llm_enabled: bool = True
    archive_raw_html: bool = True
    capture_dom_fingerprint: bool = True
    capture_device_stamp: bool = True
    screenshot_on_failure: bool = False
    manual_review_on_challenge: bool = True
    max_browser_actions: int = Field(default=10, ge=0, le=50)
    max_seconds_per_page: int = Field(default=30, ge=5, le=180)
    max_pages_per_domain: int = Field(default=0, ge=0, le=5000)
    recipe_set: Literal["generic", "business", "restaurant", "clinic", "directory"] = "business"
    resource_profile: Literal["low", "normal", "high"] = "normal"
    worker_count: int = Field(default=0, ge=0, le=128)  # 0 = auto
    ...
    social_auth_mode: Literal["public", "authenticated"] = Field(
        default="public",
        description="Use public pages only, or attach a saved browser login session for selected social platforms.",
    )
    social_auth_platforms: list[SocialPlatform] = Field(
        default_factory=lambda: [SocialPlatform.facebook, SocialPlatform.instagram]
    )
    social_auth_session_label: str = Field(default="default", min_length=1, max_length=64)
    social_auth_required: bool = False
    notes: str = ""

    @field_validator("social_auth_platforms", mode="before")
    @classmethod
    def normalize_social_auth_platforms(cls, value: Any) -> list[str]:
        if value in (None, ""):
            return []
    ...
    @field_validator("social_auth_session_label")
    @classmethod
    def normalize_social_auth_session_label(cls, value: str) -> str:
