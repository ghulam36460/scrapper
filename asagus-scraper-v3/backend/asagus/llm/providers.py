...
EXTRACTION_SYSTEM_PROMPT = (
    ...
    "linkedin_url, category, decision_makers, "
    "confidence. Use empty strings when unknown and confidence from 0 to 1."
    " decision_makers must be a list of public professional contacts only, each with"
    " name, title, profile_url, email, phone, and evidence when present."
)
...
class LLMClient:
    ...
    async def extract_business(self, text: str, source_url: str) -> ExtractedRecord | None:
        ...
        return ExtractedRecord(
            ...
            raw_fields={
                "llm_provider": self.settings.provider.value,
                "llm_model": self.settings.model,
                "decision_makers": payload.get("decision_makers") if isinstance(payload.get("decision_makers"), list) else [],
            },
    ...
    async def test_connection(self) -> tuple[bool, str]:
        if not self.enabled:
            return False, "Provider is not enabled. Select provider, model, base URL if needed, and key if required."
        try:
            payload = await self._complete_json(
                system="Return JSON only: {\"ok\":true,\"message\":\"ready\"}.",
                user="Test the provider connection.",
                raise_errors=True,
            )
            if payload:
                return True, str(payload.get("message") or "Provider responded with JSON.")
            return False, "Provider responded, but no JSON object could be parsed."
        except Exception as exc:
            return False, str(exc)

    async def _complete_json(self, system: str, user: str, raise_errors: bool = False) -> dict[str, Any] | None:
        try:
