...
async def main() -> None:
    ...
    from asagus.main import planned_page_count, _discovery_refill, useful_record, effective_runtime_flags
    ...
    s = Settings(enable_network_fetch=False, enable_search_discovery=False)
    ...
    # -------------------------------------------------------
    print("\n=== TEST 3: Per-job runtime overrides ===")
    req_real = ScrapeStartRequest(query="te", location="ci", enable_network_fetch=True, enable_search_discovery=True)
    eff_net, eff_disc = effective_runtime_flags(req_real, s)
    check("job can enable network_fetch when default False", eff_net is True)
    check("job can enable search_discovery when default False", eff_disc is True)

    req_none = ScrapeStartRequest(query="te", location="ci")  # no override
    eff_net2, eff_disc2 = effective_runtime_flags(req_none, s)
    check("None network flag follows settings False", eff_net2 is False)
    check("None discovery flag follows settings False", eff_disc2 is False)
    s_real = Settings(enable_network_fetch=True, enable_search_discovery=True)
    req_disable = ScrapeStartRequest(query="te", location="ci", enable_network_fetch=False, enable_search_discovery=False)
    eff_net3, eff_disc3 = effective_runtime_flags(req_disable, s_real)
    check("job can disable globally enabled network_fetch", eff_net3 is False)
    check("job can disable globally enabled search_discovery", eff_disc3 is False)
