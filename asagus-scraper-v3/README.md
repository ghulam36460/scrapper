...
- Download-project integration pass with optional Scrapy/Scrapling/Agent-Reach-style adapters plus native outreach-fit scoring.
- Outreach readiness metadata on every enriched record: fit score, segment, niche, recommended channel and public-presence reasons.
- Real robots.txt checks when network fetch is enabled and the job keeps `respect_robots_txt=true`; preview mode stays offline and uses local private-path guardrails.
- Local raw HTML archiving under `data/raw_html/<job-id>/` when `archive_raw_html=true`.
- Bring-your-own proxy URL slots for residential, ISP-static, datacenter and budget residential routing.
...
When real fetch is enabled, `respect_robots_txt=true` fetches and caches each domain's robots.txt before crawling. Turning the checkbox off disables that robots gate for the job, but blocklist, honeypot and rate-limit checks still apply.

Set proxy URLs only for providers and accounts you own or are authorized to use:

```dotenv
RESIDENTIAL_PROXY_URL=http://user:pass@host:port
ISP_STATIC_PROXY_URL=http://user:pass@host:port
DATACENTER_PROXY_URL=http://user:pass@host:port
BUDGET_RESIDENTIAL_PROXY_URL=http://user:pass@host:port
```

## Quick Start: Linux Without Docker
...
The scripts create `.env` from `.env.example`, create the backend virtualenv, install lightweight local dependencies from `backend/requirements-local.txt`, and start the local dev servers.

The full `backend/requirements.txt` keeps optional production/research integrations. You do not need it for the normal no-Docker GUI workflow.

Real scraping is locked by backend environment flags. To collect real public leads, set both of these in `.env`, then restart the backend:
...
The frontend can disable real fetch/discovery per job, but it can no longer turn them on unless the backend allows them.

For shared or non-private use, also set:

```dotenv
OPERATOR_TOKEN=choose-a-strong-token
```

Then enter that token in the frontend `Setup` tab.
...
2. Add production browser-pool metrics and optional Camoufox adapter behind `FetchLayer`.
...
7. Add optional hosted Firecrawl and ScrapeGraphAI backends behind explicit API-key/provider toggles.
